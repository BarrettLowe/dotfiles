...
**Route first, based on the invocation:**

| Invocation | Do this |
|---|---|
| `/graphify [path]` (no mode flag), or with `--mode deep`/`--no-viz`/`--wiki`/`--obsidian`/`--svg`/`--graphml`/`--neo4j*`/`--mcp` | Follow Steps 1-9 below (this is the default full pipeline; those flags only affect specific steps within it) |
| `/graphify <path> --update` or `--cluster-only` | Skip to "For --update and --cluster-only" near the end of this file |
| `/graphify query`, `/graphify path`, `/graphify explain` | Skip to "For /graphify query, /graphify path, /graphify explain" near the end of this file |
| `/graphify add`, `--watch`, git commit hook setup, CLAUDE.md integration | Skip to "For /graphify add, --watch, git commit hook, native CLAUDE.md integration" near the end of this file |
...
### Step 7 - Optional exports (only if --neo4j, --neo4j-push, --svg, --graphml, or --mcp given)

If none of those flags were given, skip straight to Step 8.

Otherwise, read `reference/exports.md` (in this skill's directory) and follow the section matching the flag(s) given. It covers Neo4j export/push, SVG export, GraphML export, and the MCP server.
...
Read `reference/rebuild-modes.md` (in this skill's directory) and follow the matching section. Both are alternate entry points into the same Steps 4–9 used by the default pipeline above.

---

## For /graphify query, /graphify path, /graphify explain

Read `reference/query-modes.md` (in this skill's directory) and follow the section matching the invocation. Each of these reads an already-built `graphify-out/graph.json` - they do not rebuild the graph.

---

## For /graphify add, --watch, git commit hook, native CLAUDE.md integration

Read `reference/automation.md` (in this skill's directory) and follow the section matching the invocation.
