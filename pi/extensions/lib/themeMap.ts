...
export const THEME_MAP: Record<string, string> = {
	...
	"implementation-team":"midnight-ocean",   // sequential implementation workflow
...
export function applyExtensionTheme(fileUrl: string, ctx: ExtensionContext): boolean {
	...
	//
	// primaryExtensionName() only returns non-null when pi was launched with an explicit
	// -e/--extension flag. During normal auto-discovery (plain `pi`, no -e flags), it's
	// null for every extension -- which used to fall through and let every auto-loaded
	// extension in ~/.pi/agent/extensions force its mapped theme via setTheme() on every
	// startup, clobbering the user's chosen theme in settings.json. Auto-discovered
	// extensions should never dictate the theme; only an explicitly passed -e extension
	// should.
	const primaryExt = primaryExtensionName();
	if (!primaryExt) {
		return true; // No explicit -e extension: don't touch the theme at all
	}
	if (primaryExt !== name) {
