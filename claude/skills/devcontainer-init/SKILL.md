...
description: Generate a devcontainer.local.json for an OKSI project by reading docker/dev.sh and docker/build_images.sh. Use when checking out a new project that needs a devcontainer.
...
Generate a `devcontainer.local.json` for the current project. The user uses devcontainers/devpod — the team does not, so this file is personal and lives only on the user's machine.
...
**Always add these personal mounts** (the user's dotfiles and Wayland clipboard — not in dev.sh but always needed):
```
...
Then always append these personal entries regardless of what's in `dev.sh`:
```json
...
Write to `devcontainer.local.json` in the project root. The output must be **valid JSON** — no comments, no trailing commas. Double-check for these common template artifacts before writing:
...
```bash
python3 -m json.tool devcontainer.local.json > /dev/null && echo "valid JSON"
```
...
```
Created devcontainer.local.json for <project>
  image:            <image>
  workspaceFolder:  <path>
  extra mounts:     <count> project-specific + 5 personal mounts (dotfiles + Wayland socket)
```
